...
def build_input_vector(user_inputs: dict, feature_cols: list) -> pd.DataFrame:
    """
    Convert the user's slider inputs into the full model feature vector.

    Parameters
    ----------
    user_inputs   : dict mapping signal name → today's value
    ...
    Design note
    -----------
    We initialise every feature to NaN and then fill what we know.
    LightGBM treats NaN as missing and routes them through the best split
    direction learned during training, so NaN-filling is safe here.
    The direct today's values are also used to approximate lag features
    (lag1, lag3, lag7) and rolling means — better than leaving them as NaN
    while acknowledging this is an approximation for the single-entry case.
    """
    ...
    for ui_name, base_name in DIRECT_SIGNAL_MAP.items():
        ...
        # Approximate lag and rolling features with today's value
        for suffix in ["_lag1", "_lag3", "_lag7",
                        "_roll3", "_roll7", "_delta1"]:
            key = base_name + suffix
            if key in row:
                row[key] = val if suffix != "_delta1" else 0.0

    # Composite features
    sleep = user_inputs.get("sleep_hours", 7.0)
    active = max(user_inputs.get("active_minutes", 30), 1)
    hr = user_inputs.get("resting_hr", 65)

    if "sleep_activity_ratio" in row:
        row["sleep_activity_ratio"] = sleep / active
    if "hr_above_baseline" in row:
        row["hr_above_baseline"] = 0.0   # no baseline available for single entry
